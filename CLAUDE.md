...
- **Deploy**: Cloudflare Workers static site via `wrangler deploy`. Build command: `npm run build:pages`. Output: `dist/public`. Config in `wrangler.jsonc` (DO NOT delete this file). Auto-deploys on push to main. There is NO backend server in production. Do NOT add server-side proxy routes, middleware, or Railway/server references.
